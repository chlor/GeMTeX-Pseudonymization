...
def update_tree_with_zip_codes(postal_codes, all_nodes, overpass_api):
    """
    ...
    all_nodes : dict
        A dictionary of node objects, where each node has the following attributes:
            - 'name' (str): The name of the node, used to match against area names from the query.
            - 'zip' (str): The postal code to be assigned to the node (this attribute will be updated).
            - 'is_root' (bool): Indicates if the node is a root node in the hierarchy.

    Returns
    -------
    dict
        The updated all_nodes dictionary with postal codes assigned to the 'zip' attribute of matching nodes.
    """
    ...
    for postal_code, name in postal_code_names.items():
        for node in all_nodes.values():
            # Check if the node's name contains the postal area name
    ...
    for node in all_nodes.values():
        ...
        if missing_postal_codes:
            node.zip = missing_postal_codes.pop(0)  # Remove and use the first available postal code
        else:
...
def get_osm_location_surrogate(overpass_api):
    overpass_api = overpy.Overpass()

    location_state = ["Berlin", "Rhein-Neckar-Kreis", "Baden-Württemberg", "Odenwaldkreis","nicht vorhanden staat"]
    location_country = ["Deutschland", "Österreich", "Italien","nicht vorhanden land","nicht vorhanden land 2","nicht vorhanden land 3"]
    location_city = ["Heidelberg", "Wiesloch", "Schwetzingen", "Bensheim", "Karlsruhe",  "Mühlhausen","nicht vorhanden stadt","nicht vorhanden stadt 2"]
    street_locations = ["Spandauer Str. 650", "Bergstr. 5a", "Carl-Theodor-Straße 77", "Fabrikstraße 13-18","nicht vorhanden straße 7b","nicht vorhanden straße 2 A","essigstr. "]
    postal_codes = [69120, 10178, 69168, 68723, 64625,1111111]
    ...
    all_nodes = update_tree_with_zip_codes(postal_codes, all_nodes, overpass_api)
