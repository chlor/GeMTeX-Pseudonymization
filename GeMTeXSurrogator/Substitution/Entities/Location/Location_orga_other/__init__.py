...
def remove_non_alphanumeric(input_string):
    ...
    - Keeping only alphanumerics, spaces, periods, commas, slashes, hyphens, and specific German characters (if relevant for general locations)
...
def query_similar_locations_adaptive(target_location_name, embedding_model, nn_search_model, nlp_processor, all_location_names, initial_k=10, max_k=100, step_size=10, min_matches=10):
    """
    ...
    while current_k <= max_k:
        ...
        if current_k <= max_k:
            logging.info(f"Insufficient matches ({len(current_filtered_locations)} < {min_matches}) found with k={current_k-step_size}. "
...
def get_location_surrogate(target_location_query, embedding_model, nn_search_model, nlp_processor, all_location_names, initial_k = 20, max_k = 100, min_matches = 20, temperature_for_sampling=0.1):
    """
    ...
    if not candidate_locations or probabilities.size == 0 or np.sum(probabilities) == 0:
        ...
        if candidate_locations:
            ...
            return sampled_location, np.full(len(candidate_locations), 1/len(candidate_locations)), candidate_locations, k_used
        return None, np.array([]), (), k_used
