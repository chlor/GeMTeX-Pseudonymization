            ...
            if re.search('[a-zA-Z]+', token.text):
                ...
                for form in self.dateFormatsAlpha:
                    try:
                        ...
                        idxMonth = [i for i, form in enumerate(self.dateReplMonths[month]) if
                                    parts == re.findall('\w+', re.sub(month, form, partsPars))]
                        if idxMonth:
                            newMonth = datetime.strftime(newTokenPars, '%B')
                            if len(self.dateReplMonths[newMonth]) > idxMonth[0]:
                                newPartsPars = re.findall('\w+',
                                                          re.sub(newMonth, self.dateReplMonths[newMonth][idxMonth[0]],
                                                                 datetime.strftime(newTokenPars, form)))
                            else:
                                newPartsPars = re.findall('\w+', re.sub(newMonth, self.dateReplMonths[newMonth][0],
                                                                        datetime.strftime(newTokenPars, form)))
                            c = 0
